...
! D14SE added by Jiaxu Zhang, UW-Madison; ported by Chengfei He, UW-Madison,
! July, 2016
! D14ICE added by Changfei He, July, 2016 (removed in the released version)
! !DESCRIPTION:
! D14SE is D14 at sea surface in equilibrium;
! !REVISION HISTORY:
!  SVN:$Id: $
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind), parameter :: &
      ...
      abio_dic14se_ind  =  3    ! Abiotic DIC14 
...
!-----------------------------------------------------------------------
   ...
   type(ind_name_pair), dimension(abio_dic_dic14_tracer_cnt) :: &
      ...
      ind_name_pair(abio_dic14se_ind, 'ABIO_DIC14SE')/)
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_ABIO_D14CSEocn           ! tavg id for (DIC14SE/DIC -1)* 1000
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      tavg_ABIO_DIC14SE_RESET_TEND ! tavg id for surface reset tendency 
                                    ! of ABIO_DIC14SE
...
!-----------------------------------------------------------------------
   ...
   vflux_flag(abio_dic14se_ind) = .true.
...
!-----------------------------------------------------------------------
   ...
   case ('restart', 'ccsm_continue', 'ccsm_branch', 'ccsm_hybrid' )
      ...
      if (abio_use_nml_surf_vals) then
         ...
         surf_avg(abio_dic14se_ind) = abio_surf_avg_dic14_const
...
!-----------------------------------------------------------------------
  ...
  call define_tavg_field(tavg_ABIO_D14CSEocn,'ABIO_D14CSEocn',3,              &
                          long_name='Abiotic oceanic Delta C14SE in permil',&
                          units='permil', grid_loc='3111',                &
                          coordinates='TLONG TLAT z_t time')
  
  call define_tavg_field(tavg_ABIO_DIC14SE_RESET_TEND,'ABIO_DIC14SE_RESET_TEND',2,              &
                          long_name='surface reset tendency of ABIO_DIC14SE',&
                          units='permil', grid_loc='3111',                &
                          coordinates='TLONG TLAT time')
!-----------------------------------------------------------------------
!EOC
...
!-----------------------------------------------------------------------
   ...
   real (r8), dimension(nx_block,ny_block) :: &
      ...
      DIC14SE_loc, &        ! local copy of model DIC14SE
      DIC_loc, &          ! local copy of model DIC
      ABIO_D14Cocn, &        ! D14Cocn value
      ABIO_D14CSEocn       ! D14CSEocn value
...
!-----------------------------------------------------------------------
   ...
   DIC14SE_loc = max(c0, p5*(TRACER_MODULE_OLD(:,:,k,abio_dic14se_ind) + &
                           TRACER_MODULE_CUR(:,:,k,abio_dic14se_ind)))
   ...
   DTRACER_MODULE(:,:,abio_dic14se_ind) = -c14_lambda_inv_sec * DIC14SE_loc
   ...
   elsewhere
      ...
      ABIO_D14CSEocn =  (DIC14SE_loc / DIC_loc -1) *1000

   end where

   call accumulate_tavg_field(ABIO_D14Cocn,tavg_ABIO_D14Cocn,bid,k)
   call accumulate_tavg_field(ABIO_D14CSEocn,tavg_ABIO_D14CSEocn,bid,k)
...
!EOC
 ...
 subroutine abio_dic_dic14_reset(TRACER_MODULE, bid)
! !DESCRIPTION:
!  reset surface value for ABIO_DIC14SE
!
! !REVISION HISTORY:
!  same as module
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   real(r8), dimension(nx_block,ny_block,km,abio_dic_dic14_tracer_cnt), &
      intent(inout) :: &
      TRACER_MODULE      ! ABIO_DIC tracers
...
!-----------------------------------------------------------------------      
!  Put CO2 and D14C data (contant, from files read in _init, or 
!  from coupler) on grid 
!-----------------------------------------------------------------------
      ...
      case ('coupler')
         call comp_varying_D14C(bid, data_ind_d14c(bid),D14C)
...
!-----------------------------------------------------------------------
!  Calculate R14C_atm and reset surf value of ABIO_DIC14SE2
!-----------------------------------------------------------------------

   if (mix_pass /= 1) then
      if (accumulate_tavg_now(tavg_ABIO_DIC14SE_RESET_TEND)) then
            WORK = -TRACER_MODULE(:,:,1,abio_dic14se_ind) / c2dtt(1)
            WORK = WORK * (c1 + PSURF(:,:,newtime,bid)/grav/dz(1))
            call accumulate_tavg_field(WORK,tavg_ABIO_DIC14SE_RESET_TEND,bid,1)
      endif
   endif

   TRACER_MODULE(:,:,1,abio_dic14se_ind) = &
            TRACER_MODULE(:,:,1,abio_dic_ind) * R14C_atm
    
 end subroutine abio_dic_dic14_reset
