...
!  
!  The ratios of water isotopes are treated as passive tracers.
!     e.g., R18O = H218O/H216O (unitless)
!  The units of surface fluxes for these tracers are cm/s
!
! !DESCRIPTION:
! ECB adding the case 'coupler' fields
! Add marginal sea balancing for surface tracer fluxes, and
!     change tracer from delta to ratio under coupled mode
...
!-----------------------------------------------------------------------
   ...
   type(ind_name_pair), dimension(wiso_tracer_cnt) :: &
      ind_name_table = (/ &
      ind_name_pair(d18o_ind, 'R18O'), &
      ind_name_pair(dD_ind, 'RHDO') /)
...
!-----------------------------------------------------------------------
   ...
   do n = 1, wiso_tracer_cnt
      ...
      tracer_d_module(n)%units      = 'unitless'
      tracer_d_module(n)%tend_units = '1/s'
      tracer_d_module(n)%flux_units = 'cm/s'
   end do
...
!-----------------------------------------------------------------------
   ...
   case ('ccsm_startup', 'one', 'ccsm_startup_spunup')
      TRACER_MODULE = c1
      if (my_task == master_task) then
          write(stdout,delim_fmt)
          write(stdout,*) ' Initial 3-d WISO ratios set to all ones'
          write(stdout,delim_fmt)
      endif
...
!-----------------------------------------------------------------------
   ...
   real (r8), dimension(nx_block,ny_block) :: &
      ...
      STF_PREC_18O,    & ! Ratio Prec flux from driver  
      STF_PREC_HDO,    & ! Ratio Prec flux from driver 
      STF_EVAP_18O,    & ! Ratio Evap flux from driver 
      STF_EVAP_HDO,    & ! Ratio Evap flux from driver 
      STF_MELT_18O,    & ! Ratio Melt flux from driver 
      STF_MELT_HDO,    & ! Ratio Melt flux from driver 
      STF_ROFF_18O,    & ! Ratio Runoff flux from driver 
      STF_ROFF_HDO,    & ! Ratio Runoff flux from driver 
      STF_IOFF_18O,    & ! Ratio Runoff flux from driver 
      STF_IOFF_HDO,    & ! Ratio Runoff flux from driver 
      STF_18O,         & ! Total ratio flux from driver
...
!-----------------------------------------------------------------------
!  calculate surface fluxes for ratio tracers
!-----------------------------------------------------------------------

            STF_PREC_18O = (PREC_18O_F - PREC_16O_F) * fwmass_to_fwflux
            STF_EVAP_18O = (EVAP_18O_F - EVAP_16O_F) * fwmass_to_fwflux
            STF_MELT_18O = (MELT_18O_F - MELT_16O_F) * fwmass_to_fwflux
            STF_ROFF_18O = (ROFF_18O_F - ROFF_16O_F) * fwmass_to_fwflux
            STF_IOFF_18O = (IOFF_18O_F - IOFF_16O_F) * fwmass_to_fwflux

            STF_PREC_HDO = (PREC_HDO_F - PREC_16O_F) * fwmass_to_fwflux
            STF_EVAP_HDO = (EVAP_HDO_F - EVAP_16O_F) * fwmass_to_fwflux
            STF_MELT_HDO = (MELT_HDO_F - MELT_16O_F) * fwmass_to_fwflux
            STF_ROFF_HDO = (ROFF_HDO_F - ROFF_16O_F) * fwmass_to_fwflux
            STF_IOFF_HDO = (IOFF_HDO_F - IOFF_16O_F) * fwmass_to_fwflux
...
!-----------------------------------------------------------------------
            ...
            STF_18O = STF_PREC_18O + STF_EVAP_18O + STF_MELT_18O +  &
                      ...
                      wiso_sflx_correction_18o * fwmass_to_fwflux
            STF_HDO = STF_PREC_HDO + STF_EVAP_HDO + STF_MELT_HDO +  &
                      STF_ROFF_HDO + STF_IOFF_HDO +                 & 
                      wiso_sflx_correction_hdo * fwmass_to_fwflux
...
!-----------------------------------------------------------------------
            ...
            ROCE_16O = c1
            ROCE_18O = p5*(SURF_VALS_OLD(:,:,d18o_ind,iblock) + &
                            SURF_VALS_CUR(:,:,d18o_ind,iblock))
            ROCE_HDO = p5*(SURF_VALS_OLD(:,:,dD_ind,iblock) + &
                            SURF_VALS_CUR(:,:,dD_ind,iblock))
...
!-----------------------------------------------------------------------
   ...
   if (vflux_flag(ind)) then
      wiso_tracer_ref_val = surf_avg(ind) - c1
   else
